...
const forgetPassword = async (req, res, { userModel }) => {
  ...
  const BillkolosSoft_app_email = settings['BillkolosSoft_app_email'];
  const BillkolosSoft_base_url = settings['BillkolosSoft_base_url'];

  const url = checkAndCorrectURL(BillkolosSoft_base_url);
  ...
  await sendMail({
    ...
    name: user.name,
    link,
    subject: 'Reset your password | BillkolosSoft',
